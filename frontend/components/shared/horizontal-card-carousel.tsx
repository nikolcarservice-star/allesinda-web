...
import { MarketplaceItemCard, type MarketplaceItemCardProps, type CardVariant } from "@/components/shared/marketplace-item-card"
    ...
    <div className={cn("relative w-full", containerClassName)}>
      ...
      >
        {items.map((item, index) => {
          const key = `${item.type}-${index}-${(item.data as { id?: string | number })?.id ?? index}`
          // Responsive card widths - use custom if provided, otherwise use default
          const cardWidth = cardWidthClassName || "w-[calc(50%-0.375rem)] sm:w-[calc(33.333%-0.375rem)] md:w-[calc(25%-0.375rem)] lg:w-[calc(20%-0.375rem)] xl:w-[calc(16.666%-0.375rem)]"
          ...
          return (
            ...
            >
              <MarketplaceItemCard
                {...item}
                variant={cardVariant ?? item.variant}
                className={cn(item.className, cardClassName)}
              />
            </div>
          )
        })}
