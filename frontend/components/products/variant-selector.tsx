...
import { getStoreProductId } from '@/lib/store/utils';
...
export function VariantOptionSelector({ option, variant, product }: VariantOptionSelectorProps) {
  ...
  const isTargetingProduct = isProductPage || activeProductId === getStoreProductId(product.id);

  const handleSelect = (valueName: string) => {
    startTransition(() => {
      setSelectedValue(valueName);
      if (!isProductPage) {
        setActiveProductId(getStoreProductId(product.id));
      }
    });
  };
...
export const useProductImages = (product: Product | CartProduct, selectedOptions?: SelectedOptions) => {
  ...
  // This enables store products to show different images when variants are selected
  ...
  // Then try images matched by alt text (for store products with 2+ variants)
